...
5. Name publish tags with the `shape-*` convention so consumers can target individual resource groups. The Composer name stays `onelegstudios/laravel-shape`, but runtime identifiers use the short `shape` name.
...
- Add a publishable migration: place the migration in `database/migrations`, wire it through a console-guarded `publishesMigrations` call with a `shape-migrations` tag, and test publish behavior with Testbench.
- Wire a new publish tag by adding a `publishes` map inside the existing console-guarded publishing method and naming the tag with `shape-*`.
